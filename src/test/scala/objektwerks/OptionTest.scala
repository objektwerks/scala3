...
class OptionTest extends FunSuite {
  test("option") {
    ...
    assert( ( greaterThanZero(0) orElse Some(-1) contains -1 ) == true )

    assert( greaterThanZero(0).isEmpty == true )
    assert( greaterThanZero(1).nonEmpty == true )
    assert( greaterThanZero(1).isDefined == true )

    assert( ( greaterThanZero(1) collect { case n: Int => n * 3 } contains 3 ) == true )

    assert( greaterThanZero(1).contains(1) == true )
    assert( greaterThanZero(1).count(_ > 0) == 1 )

    assert( greaterThanZero(1).exists(_ > 0) == true )
    assert( ( greaterThanZero(1).filter(_ > 0) contains 1 ) == true )

    assert( greaterThanZero(1).forall(_ > 0) == true )
  ...
  test("map") {
    assert( ( greaterThanZero(1) map(_ * 3) getOrElse(-1) ) == 3 )
  }
