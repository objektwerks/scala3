...
class InfixTest extends AnyFunSuite with Matchers {
  test("infix") {
    ...
    val addAmount = Amount(1.0) add Amount(2.0)
    addAmount shouldBe Amount(3.0)
    (Amount(1.0) add Amount(2.0)) shouldBe Amount(3.0)

    Amount(3.0) - Amount(2.0) shouldBe Amount(1.0)
    val subtractAmount = Amount(3.0) subtract Amount(2.0)
    subtractAmount shouldBe Amount(1.0)
